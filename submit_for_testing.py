...
def _load_template(template_name, template_path, device_type):
    ...
    if template_name:
        ...
        if os.path.exists(template_file_name):
            ...
            logger.error('template (%s) was specified but does not existi' %
                  template_file_name)
            sys.exit(1)
...
def _submit_to_squad(lava_job, lava_url_base, qa_server_api, qa_server_base, qa_token):
    ...
    try:
        ...
        if results.status_code < 300:
            logger.info("%s/testjob/%s %s" % (qa_server_base, results.text, get_job_name(lava_job)))
        else:
...
def main():
    ...
    parser.add_argument("--variables",
                        ...
                        dest="variables",
                        required=True)
    parser.add_argument("--overwrite-variables",
    ...
    parser.add_argument("--dry-run",
                        ...
                        action='store_true',
                        dest="dryrun")
    parser.add_argument("--test-lava-validity",
    ...
    if args.dryrun:
        ...
        j2_env = Environment(loader=FileSystemLoader(THIS_DIR, followlinks=True), undefined=LoggingUndefined)
    context = {}
