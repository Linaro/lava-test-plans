...
variable_input_file = "test/variables-tags.ini"
tests = []
...
def test_call_lava_tags_testcase(param):
    ...
    sys.argv = shlex.split(f'lava_test_plans --dry-run --variables "{variable_input_file}" --device-type "{device}" --test-case "{testcase}" {test_lava_validity}')
    assert main() == 0
